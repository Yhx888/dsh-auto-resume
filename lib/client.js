...
//   2. 从"断开 → 恢复"后（或页面加载时发现 localStorage 有断开记录且后端
//      已通），检查当前会话：若最后一条用户指令之后没有助手回复（即 AI 被
//      打断没跑完），自动通过 session.prompt 提交一条续接消息；
//   3. 全页面同一时刻只有一个续接执行者（localStorage 原子清除 + 模块级
//      忙标志），多会话 watcher 并发也不会重复提交。
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
     ...
     * 任何并发 watcher / 页面 reload 都只会有一个执行者拿到 true。
     */
    function claimDisc(sessionId) {
      var disc = loadDisc();
      if (disc === null || disc.sessionId !== sessionId) return false;
      clearDisc();
      return true;
    }
    ...
    function Watcher(props) {
      ...
      React.useEffect(() => {
        ...
        const boot = async () => {
          if (disposed) return;
          const disc = loadDisc();
          if (disc !== null && disc.sessionId === sessionId && (await pingBackend())) {
            // 页面 reload 后恢复：disc 还在且后端已通 → 直接续。
            if (claimDisc(sessionId)) await maybeContinue(connection, sessionId, disc.ts);
          }
        };
          ...
          if (ok) {
            if (disconnectedSince > 0) {
              ...
              if (Date.now() - discTs >= DISC_MIN_MS) {
                if (claimDisc(sessionId)) await maybeContinue(connection, sessionId, discTs);
              }
            }
